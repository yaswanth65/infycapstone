...
public sealed class AttendeeController : ControllerBase
{
    ...
    [HttpGet("events/{eventId:long}/access-link")]
    public async Task<IActionResult> GetVirtualMeetingLink(
        [FromRoute] long eventId,
        [FromServices] EventManagementServiceDAL.Repositories.AttendeeRepo.IRegistrationRepository regRepo,
        [FromServices] EventManagementServiceDAL.Repositories.EventManagerRepo.IEventRepository eventRepo,
        CancellationToken ct)
    {
        ...
        var reg = await regRepo.GetByEventAndAttendeeAsync(eventId, attendeeUserId, ct);
        if (reg is null || reg.RegistrationStatus != "Confirmed")
        {
            return Forbid();
        }

        var ev = await eventRepo.GetByIdAsync(eventId, ct: ct);
        if (ev is null) return NotFound();

        return Ok(new { eventId = ev.EventId, title = ev.Title, isVirtual = ev.IsVirtual, virtualMeetingUrl = ev.VirtualMeetingUrl });
    }
